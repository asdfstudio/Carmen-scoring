          ...
          Swal.fire({
            ...
            html: type === 'edit' ?
                   `<p><b>${result.value.edited}</b> class has been <b><i>saved!</i></b></p>
                    <p><b>${result.value.new}</b> class has been <b><i>created</i></b> successfully!</p>`
                  :
...
(function ($) {
  "use strict";
  $('#organization-vote').change(function () {
    const token = $('meta[name="_token"]').attr('content');
    const organization_id = $(this).data('organization');
    let vote_setting = 0;

    if ($('#organization-vote').prop("checked") == true) {
      vote_setting = 1
    }

    $.ajax({
      url: '/organizer/vote-setting',
      type: 'POST',
      data: {
