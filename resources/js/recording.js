...
function AudioRecorder(element) {
  ...
  this.micRecorder =
    this.mode === "recorder" ? new MicRecorder({ bitRate: 128 }) : null;
  ...
  this.playlistItemHTML = recording => {
    var maybeDeleteButton = this.canDelete
      ? '<button class="ar-playlist-delete" title="Delete Recording"></button>'
      : "";
    return `<li id="recording-${recording.id}" data-id="${
      recording.id
    }" data-url="${recording.url}">
              <audio><source src="${recording.url}"></audio>
              <div class="ar-playlist-item-name">${niceDate(
  ...
  if (this.playlist.length == 0) {
    ...
    if (this.playlistContainer.data("recordings")) {
      ...
      this.playlistItems = this.playlistContainer.find("ol > li");
      this.playlistPlayPause = this.playlistItems.find(
    ...
    this.playlistPlayPause.click(e => {
      ...
      if (!this.playlistCurrentItem.hasClass("playing")) {
        ...
        this.playbackTitleSlot.text(
          this.playlistCurrentItem.find(".recording-name").text()
        );
        ...
        this.playlistCurrentPlayPause = this.playlistCurrentItem.find(
          ".ar-playlist-play-pause"
        );
        ...
        $(this.playlistCurrentAudio).on("timeupdate", e => {
          var formattedTime = this.formatTime(
            this.playlistCurrentAudio.currentTime
          );
          this.progressText.text(formattedTime);
          var percent =
            (this.playlistCurrentAudio.currentTime /
              this.playlistCurrentAudio.duration) *
            100;
        ...
        if (
          this.playlistCurrentAudio.paused ||
          this.playlistCurrentAudio.ended
        ) {
          this.playlistCurrentPlayPause.addClass("active");
          this.playlistCurrentAudio.play();
        } else {
    ...
    $.ajax({
      ...
      .done((result, textStatus, jqXHR) => {
        if (!result.url) {
          ...
          this.existingLabel.text(
            this.existingRecordingCount +
              " " +
              (this.existingRecordingCount == 1 ? "Recording" : "Recordings") +
              " on File"
          );
          ...
          this.playlistDownload = this.playlistItems.find(
  ...
  this.warnRecordingSaveError = () => {
    Swal.fire({
      title: "Failed!",
      text:
        "There was an error saving your recording to the server.  Please refresh this page and try again.",
      icon: "error"
    });
  };
  ...
  this.warnUploadRecordingError = () => {
    Swal.fire({
      title: "Failed!",
      text:
        "There was an error uploading your file to the server.  Please refresh this page and try uploading the file again.",
      icon: "error"
    });
  };
