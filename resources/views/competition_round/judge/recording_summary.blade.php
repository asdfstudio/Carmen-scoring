...
@section('content')
  ...
  @if($round->targets->count() > 0)
    <div class="alert alert-info">
      <h4>This type feeds into <strong>{{ $round->targets->first()->division->name }}, {{ $round->targets->first()->name }}</strong>  along with {{ $round->targets->first()->sources->count() }} other type(s).</h4>
