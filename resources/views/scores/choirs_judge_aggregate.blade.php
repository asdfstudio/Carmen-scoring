...
<table class="table scoreboard last-col-right">
  ...
  <tr>
    ...
    <td>
      ...
      @if($round->is_scoring_active == true && $judge_id == Auth::user()->person_id && $competition->organization->is_premium == 1)
      @php $recording_count = $choir->recordings->count(); @endphp
      <div class="audio-recorder" id="audio-recorder-{{ $choir->id }}" data-mode="recorder" data-count="{{ $recording_count }}" data-choir="{{ $choir->id }}" data-round="{{ $round->id }}" data-division="{{ $round->division_id }}">
